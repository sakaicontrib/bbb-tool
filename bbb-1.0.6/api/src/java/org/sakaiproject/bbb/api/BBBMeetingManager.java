...
public interface BBBMeetingManager
{
	...
	public static final String[]	FUNCTIONS				= new String[] {
		FN_CREATE,
		FN_EDIT_OWN, FN_EDIT_ANY,
		FN_DELETE_OWN, FN_DELETE_ANY,
		FN_PARTICIPATE
	};
	 ...
	 * can participate in.
	 * 
	 * @param siteId The site to retrieve meetings for
	...
	/**
	 * Updates an existing meeting using the passed in object. Passwords
	 * and token fields of <code>meeting</code> won't be changed.
	 * 
	 * @param meeting
	 */
	...
	/**
     * Check the BigBlueButton server to see if the meeting is running (i.e. there is someone in the meeting)
     */
	...
	/**
	 * Returns true if the current user is a participant in the supplied meeting.
	 */
	public boolean isMeetingParticipant(BBBMeeting meeting);
	
	/**
	 * Returns true if the current user can create meetings in the supplied site.
	 */
	public boolean getCanCreate(String siteId);
	
	/**
	 * Returns true if the current user can edit the specified meeting in the supplied site.
	 */
	public boolean getCanEdit(String siteId, BBBMeeting meeting);
	
	/**
	 * Returns true if the current user can delete the specified meeting in the supplied site.
	 */
	public boolean getCanDelete(String siteId, BBBMeeting meeting);
	
	/**
	 * Returns true if the current user can participate on meetings in the supplied site.
	 */
	public boolean getCanParticipate(String siteId);
	
	/** Checks tool permissions in site, apply defaults if no perms set and defaults set on sakai.properties. */
	...
	/** Returns the text notice (if any) to be displayed on the first time the tool is accessed by an user. */
	public String getNoticeText();
	
	/** Returns the text notice level to be used when displaying the notice text (info | warn | success). */
