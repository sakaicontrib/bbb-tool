...
/**
 * BBBAPIWrapper is the class responsible to interact with the BigBlueButton API.
...
public class BBBAPIWrapper/* implements Runnable*/ {
	...
	public void start() {
		...
		String bbbSaltString = config.getString(BBBMeetingManager.CFG_SALT, "xxxxxx");
		...
		// Do an initial round of api mappings synchronously. This will break a Sakai startup if it
		...
		for(BBBMeeting meeting : meetings) {
			...
			if(!bbbProxyMap.containsKey(hostUrl)) {
				...
				logger.warn("'" + hostUrl + "', the host of meeting '" + meeting.getId() + "', was not available. The meeting will be moved to the first available host '" + api.getUrl() + "' ...");
				try {
		...
		/*
		bbbVersionCheckInterval = (long) config.getInt(BBBMeetingManager.CFG_VERSIONCHECKINTERVAL, (int) bbbVersionCheckInterval);
		bbbVersionCheckThreadEnabled = bbbVersionCheckInterval > 0;
		if(bbbVersionCheckThreadEnabled) {
	...
	public String getJoinMeetingURL(String meetingID, User user, String password) {
		...
		if(hostProxy == null) {
			logger.warn("No proxy found for host '" + hostUrl + ". Returning \"\" for the join url ...");
			return "";
		}
	...
	public void run(){
		try{
			...
			while(bbbVersionCheckThreadRunning){
				...
				for(BBBAPI bbbApi : bbbProxyMap.values()) {
					// do the check
					bindAPIClassToBBBVersion(bbbApi);
				}
				
				// sleep if no work to do
				if(!bbbVersionCheckThreadRunning) break;
				try{
			...
			if(bbbVersionCheckThreadRunning){
				// thread was stopped by an unknown error: restart
				logger.debug("BBB API Version Check thread was stoped by an unknown error: restarting...");
				startBBBVersionCheckThread();
			}else
	...
	*/
	...
	/*
	private void startBBBVersionCheckThread(){
		bbbVersionCheckThreadRunning = true;
		bbbVersionCheckThread = null;
		bbbVersionCheckThread = new Thread(this, this.getClass().getName());
		bbbVersionCheckThread.start();
	}
	*/
	
	/** Stop the BBB Version Check thread */
	/*
	private void stopBBBVersionCheckThread(){
		bbbVersionCheckThreadRunning = false;
		synchronized (bbbVersionCheckThreadSemaphore){
	...
	private class AllocatorTimerTask extends TimerTask {
		...
		private void allocate(List<String> testUrls) {
			...
			try {
				...
				logger.error("Failed to allocate a BigBlueButton instance. The getMeetings call failed against " + bbbProxy.getUrl() + ". It will be removed from the testUrls list for this allocation run.",e);
