...
(function ($) {
	...
	meetings.utils.getMeetingList = function (siteId) {
        ...
        $.ajax( {
            ...
            success : function (m,status) {
                ...
                //    meetings.utils.setMeetingPermissionParams(list[i]);
                //}
            },
            error : function (xmlHttpRequest,status,error) {
	...
	meetings.utils.addUpdateMeeting = function () {
        ...
        } else if (FCKeditorAPI) {
            // Get description/welcome msg from CKEditor
            meetings.utils.updateFromInlineFCKEditor('bbb_welcome_message_textarea');
        }
        ...
        if( descriptionLength > maxLength ) {
            ...
            if (CKEDITOR) {
                ...
                meetings.utils.makeInlineFCKEditor('bbb_welcome_message_textarea', 'Basic', '480', '200');
            }
            errors = true;
        }
    ...
    meetings.utils.joinMeeting = function (meetingId, linkSelector, multipleSessionsAllowed) {
    ...
    meetings.utils.showMessage = function (msgBody, severity, msgTitle, hideMsgBody) {
    	...
    	if( !meetings.errorLog[msgBody] ){
	        ...
	        if(msgTitle && msgTitle != '') {
	            ...
	            if(hideMsgBody) message.append('<span id="msgShowDetails-'+id+'">&nbsp;<small>(<a href="#" onclick="$(\'#msgBody-'+id+'\').slideDown();$(\'#msgShowDetails-'+id+'\').hide();meetings.utils.adjustFrameHeight();return false;">'+bbb_err_details+'</a>)</small></span>');
	        }
	        $('<p class="closeMe">  (x) </p>').click(function (){ meetings.utils.hideMessage(msgId); }).appendTo(message);
	        if(msgBody) {
	        ...
	        msgDiv.html(message);
	        msgsDiv.fadeIn(function (){ meetings.utils.adjustFrameHeight(); });
    ...
    meetings.utils.adjustFrameHeight = function () {
    ...
    meetings.utils.makeInlineFCKEditor = function (textAreaId, toolBarSet, width, height) {
        ...
        var applyFCKEditor = function () {
            ...
            var collectionId = '/group/' + meetings.startupArgs.siteId + '/';
    ...
    /** Update data from inline FCKEditor */
    meetings.utils.updateFromInlineFCKEditor = function (textAreaId) {
...
meetings.utils.setNotifictionOptions = function () {
    ...
    if ( $('#notifyParticipants')[0].checked ) {
        ...
        .html('<br>' + bbb_notification_notify_ical + '&nbsp;<input id="iCalAttached" name="iCalAttached" type="checkbox" checked="checked" onclick="meetings.utils.setNotifictioniCalOptions();"/>&nbsp;<span id="notifyParticipants_iCalAlarm_span"></span>')
        .show();
